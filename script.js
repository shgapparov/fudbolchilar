...
function Yaratish(fudbolchilar){
    grid .innerHTML=''
    fudbolchilar.forEach(qator => {
...
Yaratish(futbolchilar)
 ...
 function qidir(){
   let qidirgan=input.value.toLowerCase()
   const topdildi = futbolchilar.filter((harbir)=>{
      return harbir.name.toLowerCase().includes(qidirgan)
   })
   Yaratish(topdildi)
 }
 ...
 function Tartib(){
    ...
    if(qiymat==="kam"){
        const natija = futbolchilar.sort((a,b)=>{
 ...
 tartib.addEventListener('change',Tartib)
 input.addEventListener('input',qidir)
  Yaratish(futbolchilar)